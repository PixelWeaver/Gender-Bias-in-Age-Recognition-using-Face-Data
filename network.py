...
def network_def(input_layer, mode):
    ...
    for filter_num, kernel_size, pool_size, dropout_rate in zip(filters,
                                                                kernel_sizes,
                                                                pool_sizes,
                                                                dropout_rates):
        ...
        pool = tf.layers.max_pooling2d(inputs=cv_layer,
                                       pool_size=[pool_size, pool_size],
                                       strides=2)

        iterative_layer = tf.layers.dropout(
            inputs=pool, rate=dropout_rate,
            training=mode == tf.estimator.ModeKeys.TRAIN)

    # Dense Layer
    flattened_pool = tf.layers.flatten(iterative_layer)
    dense_layer_common = tf.layers.dense(inputs=flattened_pool, units=1024,
                                         activation=tf.nn.relu)
    dropout_layer = tf.layers.dropout(
        inputs=dense_layer_common, rate=0.4,
        training=mode == tf.estimator.ModeKeys.TRAIN)
